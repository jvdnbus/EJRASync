...
namespace EJRASync.Lib.Services {
	public interface IS3Service : IDisposable {
		...
		Task UploadFileAsync(string bucketName, string key, string localFilePath, Dictionary<string, string>? metadata = null, IProgress<long>? progress = null, CancellationToken cancellationToken = default);
		Task UploadDataAsync(string bucketName, string key, byte[] data, Dictionary<string, string>? metadata = null, CancellationToken cancellationToken = default);
		Task UploadLargeFileAsync(string bucketName, string key, string localFilePath, IProgress<long>? progress = null, CancellationToken cancellationToken = default);
